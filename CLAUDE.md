...
- Deploy: push to `main`. GitHub Pages serves `main` / `(root)`; the custom domain is configured in the repo's Pages settings, not by a `CNAME` file in the tree.
...
- **`AudioContext` is created only inside `audioOn()`, after a user gesture.** iOS Safari blocks audio otherwise, so chimes stay silent until the first button press.
