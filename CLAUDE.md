...
2. `<style>` themes — color tokens on `:root` and `:root[data-app-theme="<id>"]`, plus the decorated-look tokens each theme carries: `--decor` / `--decor-size` / `--decor-pos` / `--decor-rep` (the backdrop layers), `--glass` / `--glass-line` / `--glass-hi` / `--glass-shadow` / `--sheen` (buttons), `--clock-shadow`, `--grain`. The `/* premium look */` block at the end of the layout styles consumes them. It is scoped to `html:not(.flat)`; `applyTheme()` sets `.flat` on `<html>` for any `THEMES` entry with `flat: true` (Midnight), which therefore keeps the plain 2D look. The backdrop paints on `body::before` / `body::after` (fixed, `z-index:-1`, `pointer-events:none`) so it can never add scroll or intercept a tap, and it is gradients and inline SVG data URIs only — no image files. Royal's chips are the background of `.stage::after`, anchored to the stage's bottom-right corner so they rest on the stats divider and stay clear of the clock and the payouts.
...
- **`drawClock()` only touches the DOM when the string changes.** The decorated clock stacks several `text-shadow`s on the largest text on the page; rewriting it on every 250 ms tick would repaint that four times a second for nothing.
...
- **New theme:** append to the `THEMES` array *and* add a matching `:root[data-app-theme="<id>"]` token block, including the decorated-look tokens (copy a dark theme's block and adjust) — or set `flat: true` on the `THEMES` entry to opt the theme out of the premium rules altogether. Swatches are generated from `THEMES`.
